...
public class ctrlFacturaMovimiento
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            switch (Integer.parseInt(bnd)) {
                ...
                case 8: // Nueva bandera para los Lotes
                    out.print(getDetLotesPrd(request));
                    break;
            }
    ...
    public static String getDetLotesPrd(HttpServletRequest request) {
    String callback = Utilities.obtenParametro(request, "callback").replaceAll("'", "");
    String record = callback + "({total:0,recordsLotes:[]});";
    ...
    if (usr != null) {
        ...
        try {
            String idAlmacen = Utilities.obtenParametro(request, "idAlmacen").replaceAll("'", "");
            String clvProducto = Utilities.obtenParametro(request, "clvProducto").replaceAll("'", "");

            // Leemos los parámetros automáticos del scroll de ExtJS
            int start = Integer.parseInt(Utilities.obtenParametro(request, "start").equals("") ? "0" : Utilities.obtenParametro(request, "start"));
            int limit = Integer.parseInt(Utilities.obtenParametro(request, "limit").equals("") ? "200" : Utilities.obtenParametro(request, "limit"));

            DaoHelper daoHelper = new DaoHelper();

            if (!idAlmacen.equals("") && !clvProducto.equals("")) {
                
                // A) Obtener el TOTAL absoluto de filas
                int totalRegistros = 0;
                ArrayList arrCount = daoHelper.getResultTable(SQLFacturas.getDetLotesCount(idAlmacen, clvProducto));
                if (arrCount != null && !arrCount.isEmpty()) {
                    Hashtable hCount = (Hashtable) arrCount.get(0);
                    totalRegistros = Integer.parseInt(String.valueOf(hCount.get("TOTAL")));
                }

                // B) Obtener SOLAMENTE las 200 filas correspondientes a la página actual
                ArrayList arrayLotes = daoHelper.getResultTable(SQLFacturas.getDetLotesPaginado(idAlmacen, clvProducto, start, limit));

                if (arrayLotes != null && !arrayLotes.isEmpty()) {
                    record = callback + "({total:" + totalRegistros + ",recordsLotes:[";

                    // Como la consulta ya viene recortada con 200 elementos desde Oracle, recorremos todo el arreglo
                    for (int i = 0; i < arrayLotes.size(); i++) {
                        Hashtable hashAuxLotes = (Hashtable) arrayLotes.get(i);

                        record += "{";
                        record += "racid:'" + String.valueOf(hashAuxLotes.get("RACID")) + "',";
                        record += "racprdid:'" + String.valueOf(hashAuxLotes.get("RACPRDID")) + "',";
                        record += "raclote:'" + String.valueOf(hashAuxLotes.get("RACLOTE")) + "',";
                        record += "racpedimento:'" + (hashAuxLotes.get("RACPEDIMENTO") != null ? hashAuxLotes.get("RACPEDIMENTO") : "") + "',";
                        record += "racfeccaducidad:'" + (hashAuxLotes.get("RACFECCADUCIDAD") != null ? hashAuxLotes.get("RACFECCADUCIDAD") : "") + "',";
                        record += "racreal:'" + String.valueOf(hashAuxLotes.get("RACREAL")) + "',";
                        record += "racreservado:'" + String.valueOf(hashAuxLotes.get("RACRESERVADO")) + "',";
                        record += "disponible:'" + (hashAuxLotes.get("DISPONIBLE") != null ? String.valueOf(hashAuxLotes.get("DISPONIBLE")) : "0") + "'";
                        record += "},";
                    }

                    record = record.substring(0, record.length() - 1) + "]});";
                } else {
                    record = callback + "({total:0,recordsLotes:[]});";
                }
            }
        } catch (Exception e) {
            record = callback + "({total:0,recordsLotes:[]});";
