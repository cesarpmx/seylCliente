    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (bnd.equals("1")) {
                ...
                out.print(this.ObtenerDatos(request));
...
//                out.print(this.obtenHorariosNutriologas(request));
            ...
            } else if (bnd.equals("42")) {
                out.print(this.ObtenerDatosLinea(request));
            } else if (bnd.equals("43")) {
                out.print(this.ObtenerDirecAlmacen(request));
                // out.print("{records:[{'IdDirecAlmacen': '01','DirecAlmacen': 'Prueba direccion'}]}");
            }
...
//                horaSalida = objUHV.getUhvIdHorario().getUhoHoraSalidaLv();
    ...
    public String ObtenerDatos(HttpServletRequest request) {
        String records = "{records:[]}";
        // HttpSession s = request.getSession();
        Liusuarios usr = Utilities.ReactivarSession(request);
        DaoHelper daoHelper = new DaoHelper();
        ArrayList arrayCombo = new ArrayList(0);
        arrayCombo.clear();
        // Liusuarios usr = Utilities.ReactivarSession(request);
        arrayCombo = daoHelper.getResultTable(this.obtenerQueryCombo(Integer.parseInt(Utilities.obtenParametro(request, "qry").equals("") ? "0" : Utilities.obtenParametro(request, "qry")), usr.getUsuempresa(), "familia"));
        if (arrayCombo != null) {
            if (!arrayCombo.isEmpty()) {
                records = "{records:[";
                for (int i = 0; i < arrayCombo.size(); i++) {
                    Hashtable hashAuxCombo = new Hashtable();
                    hashAuxCombo.clear();
                    hashAuxCombo = (Hashtable) arrayCombo.get(i);
                    records += "{'" + Utilities.obtenParametro(request, "idCampo") + "': '" + (String) hashAuxCombo.get("IDCAMPO") + "',";
                    records += "'" + Utilities.obtenParametro(request, "campo") + "': '" + (String) hashAuxCombo.get("CAMPO") + "'},";
                }
                records = records.substring(0, records.length() - 1) + "]}";
            }
        }
    ...
    public String ObtenerDatosLinea(HttpServletRequest request) {
        String records = "{records:[]}";
        DaoHelper daoHelper = new DaoHelper();
        ArrayList arrayCombo = new ArrayList(0);
        arrayCombo.clear();
        Liusuarios usr = Utilities.ReactivarSession(request);
        String linea = Utilities.obtenParametro(request, "familia");
        if (linea == null || linea.equals("") || linea.equals("null")) {
            linea = null;
        }
        else {
            linea = "'"+linea+"'";
        }
        arrayCombo = daoHelper.getResultTable(this.obtenerQueryCombo(Integer.parseInt(Utilities.obtenParametro(request, "qry").equals("") ? "0" : Utilities.obtenParametro(request, "qry")), usr.getUsuempresa(), linea));
        if (arrayCombo != null) {
            if (!arrayCombo.isEmpty()) {
                records = "{records:[";
                for (int i = 0; i < arrayCombo.size(); i++) {
        ...
        public String ObtenerDirecAlmacen(HttpServletRequest request) {
        String records = "{records:[{'DirecAlmacen': 'Ningun almacén seleccionado'}]}";
        String query1;
        DaoHelper daoHelper = new DaoHelper();
        ArrayList arrayCombo = new ArrayList(0);
        arrayCombo.clear();
        Liusuarios usr = Utilities.ReactivarSession(request);
        String clave = Utilities.obtenParametro(request, "familia");
        ...
        query1 = "SELECT almid as IDCAMPO, ";
        query1 += "(almdirec1 || ' ' || almnumext || ' ' || almnumint || ' COL. ' || almdirec2 || ', ' || almdirec3 || ', C.P.' || almcp || ', ' || almestado || ', ' || almpais) AS CAMPO ";
        query1 += "from sealmlogicos, sealmacenes where (aloid = "+clave+") and almid = aloalmid ";
        
        arrayCombo = daoHelper.getResultTable(query1);
        if (arrayCombo != null) {
            if (!arrayCombo.isEmpty()) {
                records = "{records:[";
                for (int i = 0; i < arrayCombo.size(); i++) {
        ...
        // hashAuxCombo.clear();
        // hashAuxCombo = (Hashtable) arrayCombo.get(i);
        // records += "{'" + Utilities.obtenParametro(request, "campo") + "': '" + clave + "'},";
        // records = records.substring(0, records.length() - 1) + "]}";
    ...
    public String obtenDatosComboActporAte(HttpServletRequest request) {
        ...
        return records.replace("ADMINISTRACIï¿½N", "COORDINACIï¿½N");
    }
    //
    ...
    public String obtenDatosComboActporAtenOrigen(HttpServletRequest request) {
        ...
        return records.replace("Coordinaciï¿½n Pen", "Coordinaciï¿½n");
    }
    //
    ...
    private String obtenerQueryCombo(int i, int Empresa, String familia) {
        String query = "";
        switch (i) {
            case 1:
                query = SQLCombo.obtenerFamilia(String.valueOf(Empresa));
                break;
            case 2:
                query = SQLCombo.obtenerLinea(String.valueOf(Empresa), familia);
                break;
            case 3:
                query = SQLCombo.obtenerTipoProd(String.valueOf(Empresa));
                break;
            case 4:
                query = SQLCombo.obtenerRegion(String.valueOf(Empresa));
                break;
            case 5:
                query = SQLCombo.obtenerPorteo();
                break;
            case 6:
                query=SQLCombo.obtenerTipoClt();
                break;
            case 7:
                query = SQLCombo.obtenerNoCiclo();
                break;
            case 8:
                query = SQLCombo.obtenerAcciones();
                break;
            case 9:
                query = SQLCombo.obtenerAlmacen(String.valueOf(Empresa));
                break;
            case 10:
                query = SQLCombo.obtenerAlmacenFisico(String.valueOf(Empresa));
                break;
            case 11:
    ...
    private String obtenQueryCombo(int i) {
        ...
        switch (i) {
            ...
            case 5:
                ...
                break;
            case 6:
                query = SQLCombo.obtenerAlmacen("");
                break;
            case 7:
                query = SQLCombo.obtEstEmbarque();
                break;
            case 8:
                query = SQLCombo.obtenerFamilia(query);
                break;
            case 9:
                query = "SELECT ld FROM Lada ld WHERE ld.lEstado like ?1 AND ld.lCiudad like ?2";
                break;
            case 10:
                query = "SELECT almdirec1 || ' ' || almnumext || ' ' || almnumint || ' ' || almdirec2 || ' ' || almdirec3 where almid = ?1";
                break;
            case 11:
