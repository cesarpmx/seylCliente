...
public class SQLCombo
{
  ...
  {
    return "Select up.upidpuesto,up.upnombrepuesto From Lipuestoorigen po, Liusuariopuesto up Where po.poidpuesto.upidpuesto=up.upidpuesto And po.poidorigen.oidorigen = ?1 And up.uphabilitado = 1";
  }
  ...
  {
    return "select a.aIdActividad,a.aNombreActividad from CtlActividad a where a.aIdTipoActividad.atiIdTipoActividad=6 ORDER BY a.aNombreActividad asc";
  }
  ...
  {
    return "Select o.oIdOrigen, o.oNombreOrigen from CtlOrigen o where o.oIdAreaAtencion.caaIdAreaAtencion=2";
  }
  ...
  public static String obtenProductoFamiliaNo(List lista)
  {
    ...
    for (int i = 0; i < lista.size(); i++) {
      consulta = consulta + "f.pfaIdProductoFamilia <> " + lista.get(i) + " and ";
    }
  ...
  {
    String consulta = "Select p.pIdProducto, CONCAT(p.pNombreProducto,' Dosis:',p.pDosis,' Present:',p.pPresentacion) From Producto p Where p.pIdProductoFamilia.pfaIdProductoFamilia=?1 and p.pStatus = 'Activo' and ";
    for (int i = 0; i < lista.size(); i++) {
      consulta = consulta + "p.pIdProducto <> " + lista.get(i) + " and ";
    }
    consulta = consulta.substring(0, consulta.length() - 4);
    return consulta;
  }
  ...
  {
    return "Select u.uIdUsuario,u.uNombre From Usuario u Where u.uIdOrigen.oIdOrigen = ?1 And u.uHabilitado = True";
  }

  public static String obtenTipoTelContacto()
  {
    return "SELECT op.coidcomboopcion,op.conombrecomboopc FROM Licomboopciones op WHERE op.coidcombo=7       AND op.cohabilitado=1  ORDER BY op.coorden asc";
  }
  ...
  {
    return "Select m.mIdMensajeria, m.mNombreMensajeria From Mensajeria m Where m.mIdMensajeria <> 7     And m.mIdEnvioTipo.etIdEnvioTipo = ?1     And m.mActivo = true Order by m.mOrden asc";
  }

  public static String obtenComboMedicamentosDiabeticosYOtros()
  {
    return "select p.pfaIdProductoFamilia,p.pfaNombreFamilia from ProductoFamilia p where p.pfaIdProductoFranquicia.cpfIdProductoFranquicia=4 and p.pfaNombreFamilia<>'Optipen'  and p.pfaNombreFamilia<>'Autopen' or p.pfaNombreFamilia='Otros' Group by p.pfaIdProductoFamilia,p.pfaNombreFamilia ORDER BY p.pfaIdProductoFamilia,p.pfaNombreFamilia asc";
  }
  ...
  public static String obtenerAcciones()
  {
    return "SELECT pacid as IDCAMPO, pacnombre as CAMPO FROM liprdacciones ac ORDER BY ac.pacnombre";
  }
  ...
  {
    return "select aloid as idCampo, alonombre as campo from sealmlogicos order by aloid";
  }

  public static String obtEstEmbarque()
  {
    return "select eemid as idCampo, eemnombre as campo from seestembarque order by eemid";
  }

  public static String obtenerFamilia(String empresa)
  {
    return "SELECT linid AS IDCAMPO, linnombre AS CAMPO FROM selineas where linempid =" + empresa + " order by linnombre ";
  }
  ...
  {
    return "SELECT almid AS IDCAMPO, almnombre AS CAMPO FROM sealmacenes where almwebvisible = 'S' order by almid ";
  }

  public static String obtenerLinea(String empresa, String linea_combo)
  {
    return "SELECT famid AS IDCAMPO, famnombre AS CAMPO FROM sefamilias WHERE famempid =" + empresa + " and (" + linea_combo + " is null or famlinid =" + linea_combo + ") order by famnombre ";
  }

  public static String obtenerTipoProd(String empresa)
  {
    return "SELECT tprclave AS IDCAMPO, tprdescripcion AS CAMPO FROM litipoproducto WHERE tprempresa =" + empresa + " order by tprdescripcion ";
  }

  public static String obtenerRegion(String empresa)
  {
    return "SELECT regclave AS IDCAMPO, regdescripcion AS CAMPO FROM liregiones WHERE regempresa =" + empresa + " order by regdescripcion ";
  }

  public static String obtenerTipoClt()
  {
    return "SELECT tclclave AS IDCAMPO, tcldescripcion AS CAMPO FROM litipocliente WHERE (tclwebvisible = 'S') order by tcldescripcion ";
  }

  public static String obtenerPorteo()
  {
    return "SELECT porclave AS IDCAMPO, pornombre AS CAMPO FROM liporteos WHERE (porwebvisible = 'S') order by pornombre ";
  }

  public static String obtenerNoCiclo()
  {
    return "SELECT CICCLAVE AS IDCAMPO,CICDESCRIPCION AS CAMPO FROM LICICLOS WHERE cicfechafin > sysdate-75 ORDER BY CICCLAVE ";
  }
}
