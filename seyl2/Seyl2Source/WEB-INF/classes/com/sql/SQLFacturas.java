...
public class SQLFacturas {
    ...
    // BIEN (Sin el ROWNUM <= 400)
// 1. Obtiene el TOTAL real de registros (Súper rápida)
    public static String getDetLotesCount(String idAlmacen, String clvProducto) {
        return "SELECT COUNT(1) AS TOTAL "
                + " FROM serack "
                + " WHERE racaloid = '" + idAlmacen + "' "
                + "   AND racprdid = '" + clvProducto + "'";
    }

// 2. Obtiene ÚNICAMENTE los 200 registros de la página solicitada
    public static String getDetLotesPaginado(String idAlmacen, String clvProducto, int start, int limit) {
        int maxRows = start + limit;
        int minRows = start;

        
        
        return "SELECT racid, racprdid, raclote, racpedimento, "
            + "TO_CHAR(racfeccaducidad, 'DD/MM/YYYY') as racfeccaducidad, "
            + "racreal, racreservado, "
            + "(racreal - racreservado) AS disponible "
            + "FROM serack "
            + "WHERE racaloid = '" + idAlmacen + "' "
            + "  AND racprdid = '" + clvProducto + "' "
            + "  AND racreal > 0 "
